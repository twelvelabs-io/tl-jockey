	...
	return (
		<div className={'w-[680px]'}>
			<div className={'flex flex-row gap-2 items-center'}>
				...
				<div className={'font-aeonikBold'}>{isUserMessage && 'You'}</div>
			</div>
			<div className={`ml-[40px] mr-[194px] whitespace-pre-line ${isUserMessage ? 'userBubble' : 'aiBubble'}`}>{message.text}</div>
			<div className={`flex flex-row gap-2 items-center mt-3 relative `}>
              ...
              <Loading/>
				...
				{<ThinkingComponent searchTerm={message?.asrTest} isLoading={loading} lastElement={lastElement} />}
			</div>
		</div>
	)
}
