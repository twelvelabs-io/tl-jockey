...
const ModalCentral: React.FC<ModalCentralProps> = ({handleClose}) => {
	...
	let textForModal =
		modalType === ModalType.MESSAGES
			? arrayOfChoosedElements?.[chosenIndex as number]?.video_title
			: findElementById(panelVideosList, chosenIndex)?.metadata.filename

	if (modalType === ModalType.CLEAR_CHAT) {
		return <ModalClear showModal={showModal} handleClose={handleClose} />
	}
		...
		<Modal size="lg" show={showModal} onHide={handleClose} centered scrollable>
			<Modal.Body className={'p-[24px]'}>
				<div className={'flex flex-col'}>
					<div>
						<QuestionHeader handleClose={handleClose} logo={union} text={textForModal} />
					</div>
					<div>
						<ReactHlsPlayer
